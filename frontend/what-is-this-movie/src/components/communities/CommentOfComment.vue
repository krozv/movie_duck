...
const createReply = () => {
    axios.post(`${store.API_URL}/api/movie/${props.movieId}/comment/${props.commentId}/`, {
...
const fetchReplies = () => {
  axios.get(`${store.API_URL}/api/movie/${props.movieId}/comment/${props.commentId}/`, {
    headers: { Authorization: `Token ${token}` }
  })
    .then(response => {
  ...
  const editReply = (reply) => {
    ...
    if (updatedContent !== null) {
      axios.put(`${store.API_URL}/api/movie/${props.movieId}/comment/${props.commentId}/${reply.id}/`, {
  ...
  const deleteReply = (replyId) => {
    if (confirm('정말 대댓글을 삭제하시겠습니까?')) {
      axios.delete(`${store.API_URL}/api/movie/${props.movieId}/comment/${props.commentId}/${replyId}/`, {
        headers: { Authorization: `Token ${token}` }
      })
      .then(response => {
